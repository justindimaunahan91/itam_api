...
class Asset extends Controller {
    function retrieveAssets() {
       ...
       CO.asset_condition_name, S.status_name 
		FROM `unmg-workplace`.itam_asset A
		LEFT JOIN `unmg-workplace`.itam_asset_category C ON A.category_id = C.category_id
		LEFT JOIN `unmg-workplace`.itam_asset_sub_category SC ON A.sub_category_id = SC.sub_category_id
		LEFT JOIN `unmg-workplace`.itam_asset_type T ON A.type_id = T.type_id
		LEFT JOIN `unmg-workplace`.itam_asset_condition CO ON A.asset_condition_id = CO.asset_condition_id
		LEFT JOIN `unmg-workplace`.itam_asset_status S ON A.availability_status = S.status_id;");
    ...
    function insertAsset($data) {
        ...
        $success = $this->statement->execute([$asset_name, $serial_number, $category_id, $sub_category_id,  1, $type_id,$availability_status_id, $location, $specifications, $asset_amount, $warranty_duration, 0, $warranty_due_date, $purchase_date, $notes]);
