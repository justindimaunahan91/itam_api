    ...
    public function insertAsset($data)
{
    ...
    if (!empty($sub_category_name)) {
        $this->setStatement("SELECT sub_category_id, code FROM itam_asset_sub_category WHERE category_id = ? AND sub_category_name = ?");
        $this->statement->execute([$category_id, $sub_category_name]);
        $subcategory = $this->statement->fetch(PDO::FETCH_ASSOC);
